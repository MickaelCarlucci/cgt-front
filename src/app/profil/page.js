...
export default function Page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (session) {
          const [userResponse, centersResponse, activitiesResponse] = await Promise.all([
            fetch(
            ...
            fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/center/${session.user.center_id}/activities`),
          ]);
